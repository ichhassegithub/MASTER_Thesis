...
def interpolant(X, Y, Z, U, V, W, time, interpolation_method = 'cubic'):
    '''
...
def create_4D_cubic_interpolator(X, Y, Z, T, U, V, W):
    """
    Creates a true 4D cubic interpolator:
      - Time interpolation using CubicSpline (1D cubic splines for each (y, x, z) location)
      - Spatial interpolation using RegularGridInterpolator (3D cubic)
    
    Parameters:
        X, Y, Z : 1D arrays defining the spatial grid
        T       : 1D array defining time points
        U, V, W : 4D arrays (Ny, Nx, Nz, Nt) for wind velocity components
        
    Returns:
        List of 3 interpolator functions: U_interp, V_interp, W_interp
    """

    def interpolate_time(data):
        """ Applies cubic spline interpolation along time for each (y, x, z) location """
        Ny, Nx, Nz, Nt = data.shape
        interpolated_data = np.zeros((Ny, Nx, Nz, len(T)))

        for j in range(Ny):
            for i in range(Nx):
                for k in range(Nz):
                    cs = CubicSpline(T, data[j, i, k, :], bc_type='not-a-knot')  # 1D Cubic Spline
                    interpolated_data[j, i, k, :] = cs(T)  # Evaluate spline at given T points
        
        return interpolated_data

    # Step 1: Apply cubic spline interpolation in time
    U_interp_time = interpolate_time(U)
    V_interp_time = interpolate_time(V)
    W_interp_time = interpolate_time(W)

    # Step 2: Apply cubic interpolation in space using RegularGridInterpolator
    U_interp = RegularGridInterpolator((Y, X, Z, T), U_interp_time, method='cubic', bounds_error=False, fill_value=None)
    V_interp = RegularGridInterpolator((Y, X, Z, T), V_interp_time, method='cubic', bounds_error=False, fill_value=None)
    W_interp = RegularGridInterpolator((Y, X, Z, T), W_interp_time, method='cubic', bounds_error=False, fill_value=None)
...
# def interpolant_unsteady(X, Y, Z, U, V, W, time, interpolation_method='linear'):
#     '''
#     Unsteady wrapper for scipy.interpolate.interpn. Creates a list of interpolators for u, v and w velocities

#     Parameters:
#         X: array (Ny, Nx, Nz), X-meshgrid
#         Y: array (Ny, Nx, Nz), Y-meshgrid
#         Z: array (Ny, Nx, Nz), Z-meshgrid (non-monotonic allowed)
#         U: array (Ny, Nx, Nz, Nt), U velocity
#         V: array (Ny, Nx, Nz, Nt), V velocity
#         W: array (Ny, Nx, Nz, Nt), W velocity
#         time: array (1, Nt), time array
#         interpolation_method: Method for interpolation. Can be 'linear', 'nearest', 'cubic', etc.

#     Returns:
#         Interpolant: list (3,), U, V, and W interpolators
#     '''
#     # Prepare the grid points for interpolation, accounting for non-monotonic Z
#     grid_points = (Y[:, 0, 0], X[0, :, 0], Z[0, 0, :], time[0, :])  # Ensure proper order for interpn

#     # Stack U, V, W velocity fields into a list
#     velocity_fields = [U, V, W]

#     # Initialize the interpolant list
#     Interpolant = []

#     for velocity in velocity_fields:
#         # Interpolate using interpn for each velocity field
#         Interpolant.append(
#             lambda xi, velocity=velocity: interpn(
#                 grid_points, velocity, xi=xi, method=interpolation_method, bounds_error=False, fill_value=0
#             )
#         )
