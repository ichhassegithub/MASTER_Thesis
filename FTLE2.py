...
# y0_xz = Y_domain[0,:,:].ravel()   # only one value
# z0_xz = Z_domain[0,:,:].ravel()  


# omega_xz, test_iteration_xz = compute_FTLE(x0_xz, y0_xz, z0_xz)

# X0_xz = np.array(x0_xz).reshape(Nz, Nx)  # array (Nz, Nx)
# Y0_xz = np.array(y0_xz).reshape(Nz, Nx)  
# Z0_xz = np.array(z0_xz).reshape(Nz, Nx)  
# omega_xz = np.array(omega_xz).reshape(Nz, Nx)  

# plt.figure(figsize=(8, 6))
# contour = plt.contourf(x_domain, z_domain, omega_xz, levels=400, cmap='inferno')
...
# x0_yz = X_domain[:,0,:].ravel()  # only one value
# y0_yz = Y_domain[:,0,:].ravel()  
# z0_yz = Z_domain[:,0,:].ravel()  


# omega_yz, test_iteration_yz = compute_FTLE(x0_yz, y0_yz, z0_yz)

# X0_yz = np.array(x0_yz).reshape(Nz, Ny)  # array (Nz, Ny)
# Y0_yz = np.array(y0_yz).reshape(Nz, Ny)  
# Z0_yz = np.array(z0_yz).reshape(Nz, Ny)  

# omega_yz = np.array(omega_yz).reshape(Nz, Nx)  

# plt.figure(figsize=(8, 6))
# contour = plt.contourf(y_domain, z_domain, omega_yz, levels=4, cmap='inferno')
# plt.colorbar(contour, label="FTLE Value")
...
# z0_xy = Z_domain[:, :, 1].ravel()  # only one value
