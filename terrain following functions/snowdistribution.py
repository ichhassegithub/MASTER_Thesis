...
# plt.title('iDiv PALM', fontsize=16)
# cbar = plt.colorbar(contour)
# cbar.set_label('iDiv (1/s)', fontsize=16)
...
# plt.xlabel('X (m)', fontsize=16)
# plt.ylabel('Y (m)', fontsize=16)
# plt.title('iDiv Windninja - 45°', fontsize=16)
# cbar = plt.colorbar(contour)
# cbar.set_label('iDiv (1/s)', fontsize=16)
# cbar.ax.tick_params(labelsize=16)
# plt.xticks(fontsize=16)
...
# snow_wn, adv_term_palm, T, L

adv_term_palm = snow_wn.LAD()
lad_m_s = adv_term_palm / T

X, Y = np.meshgrid(snow_wn.x_domain * L, snow_wn.y_domain * L)
Z = lad_m_s.reshape(X.shape)

fig, ax = plt.subplots(figsize=(8, 6))

# Main contour plot
contour = ax.contourf(X, Y, Z, levels=250, cmap='viridis')
...
axins.contourf(X, Y, Z, levels=250, cmap='viridis')

# Set zoom region (adjust these based on your domain)
x1, x2, y1, y2 = 6000, 7000, 6300, 7500
...
# Optional: Add wind vectors if available
if hasattr(snow_wn, 'u') and hasattr(snow_wn, 'v'):
    ax.streamplot(X, Y, snow_wn.u, snow_wn.v, color='m', linewidth=0.5, density=3, arrowsize=0.6)

# Labels and title
ax.set_title('iDiv Windninja - 45°', fontsize=16, pad=15)
